...
export function addMovingEvent(htmlObject:HTMLElement|HTMLDivElement){
    let attributeX: string, attributeY: string
    ...
    htmlObject.addEventListener("mousedown", (e:any): void=>{
        // to stop any event not related to this element
        if(e.target !== e.currentTarget) return;

        // to allow mouse to move into other elements
        let htmlObjectParentNode  = <HTMLDivElement> htmlObject.parentNode
        Array.from(htmlObjectParentNode["children"]).forEach((p:any)=>{
