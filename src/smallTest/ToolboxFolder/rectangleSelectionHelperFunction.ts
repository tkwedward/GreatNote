...
export function getPointWidthAndHeight(rect: HTMLDivElement){
...
export function checkPointInsideRect(point: {x: number, y: number}, rectData: {x: number, y: number, w: number, h: number}){
  return point.x > rectData.x && point.x < rectData.x + rectData.w  && point.y > rectData.y && point.y < rectData.y + rectData.h
} // checkSideInsideRect

export function checkCornersInside(rect1Data: {x: number, y: number, w: number, h: number}, rect2Data: {x: number, y: number, w: number, h: number}){
...
export function checkSideInsideRect(rect1Data: {x: number, y: number, w: number, h: number}, rect2Data: {x: number, y: number, w: number, h: number}){
...
export function addEventToSelectionRect(selectionRect: HTMLDivElement, rectSelectionToolController: SelectionToolControllerInterface){
    selectionRect.addEventListener("mousedown", e=>{
        ...
        let divObjectInitialPosition = selectionRect.divLayerItemArray.map((p:HTMLDivElement)=>{
            let x = +p.style.left.replace("px", "")
            let y = +p.style.top.replace("px", "")
            return {x, y}
        })

        let svgObjectInitialPosition = selectionRect.svgLayerItemArray.map((p:any)=>{
        ...
        let mouseMoveFunction = function(mouseeMoveEvent:MouseEvent){
          e.preventDefault()
          ...
          let divCheckbox = <HTMLInputElement> rectSelectionToolController.querySelector("#divCheckBox")
          let svgCheckbox =  <HTMLInputElement> rectSelectionToolController.querySelector("#svgCheckBox")
          ...
          if (svgCheckbox.checked){
              ...
              for (let i = 0; i < svgLayerItemArray.length; i++){
                  let newPointPosition = svgObjectInitialPosition[i].map((p:[number, number])=>[p[0] + deltaX, p[1] + deltaY])
        ...
        let mouseUpFunction = function(mouseeUpEvent:MouseEvent){
