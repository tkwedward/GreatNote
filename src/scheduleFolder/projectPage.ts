import * as TaskDetailItem from './taskDetail/taskDetailItem';
import * as ScheduleHelperFunction from "./scheduleHelperFunction"
import {ProjectItemInterface, TaskItemInterface} from "./taskDetail/taskItemInterface"

interface ScheduleObject extends HTMLDivElement {
    uniqueID: string
}



let projectPageWrapper = <ScheduleObject> document.querySelector(".projectPageWrapper")
...
export function addItemToProjectPage(categoryName: string, projectItemArray: any[]){
...
//       },
//       {
//         name: "write chapter 1 summay notes",
//         status: "in progress",
//       },
//       {
//         name: "read chapter 2",
...
interface ProjectRowInterface extends HTMLDivElement{
    uniqueID: string
    update():any
}
export function createProjectRow(projectDetailJsonFile?:{
  name: string, uniqueID: string, array: any[]
}): HTMLDivElement {


    let projectRow = <ProjectRowInterface> document.createElement("div")
    projectRow.classList.add("projectRow")

    projectRow.uniqueID = projectDetailJsonFile?.uniqueID || ScheduleHelperFunction.createUniqueID()

    let projectName = document.createElement("div")
    projectName.innerText = projectDetailJsonFile?.name || "Untitled Project Name"
    projectName.contentEditable = "true"
    let block = false

    let updateFunction = function(e:any){
        projectRow.update()
...
interface TaskRowInterface extends HTMLDivElement{
    uniqueID: string
    update():any
}

function createTaskRow(taskJsonFile?: TaskItemInterface, parentIdentifier?:string): TaskRowInterface{
    let taskRow = <TaskRowInterface> document.createElement("div")
    taskRow.classList.add("taskRow")

    taskRow.uniqueID = taskJsonFile?.uniqueID || ScheduleHelperFunction.createUniqueID()
    ...
    taskName.innerText = taskJsonFile?.data.name || "UntitledTask"
    ...
    taskStatus.value = taskJsonFile?.data.status || "in progress"
    ...
    record.innerText = taskJsonFile?.data.record || "Please enter record."
    record.contentEditable = "true"
...
function renderProjectDetail(projectDetailJsonFile?: any){
